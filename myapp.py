...
st.write("""
##### Se presenta un grafico de lineas donde se puede apreciar las coincidencias en cada pico
""")
...
#Encontramos la mayor correlación
# m_corr_d_pandas matriz
def correlacion_mayor():
    #Ordenamos la matriz
    orden = m_corr_d_pandas.unstack()
    orden = orden.sort_values(kind = "quicksort",ascending = False)
    return orden[len(preferencias):len(preferencias)+4]
